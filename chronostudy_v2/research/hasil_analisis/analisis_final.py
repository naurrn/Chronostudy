...
analisis_final.py
==================
Jalanin setelah export_data_mentah.py, di folder yang sama.

    python analisis_final.py

Alurnya PERSIS yang kita kerjain manual sebelumnya:
    1. Baca data_mentah_terbaru.csv
    2. Buang identitas dummy/testing (edit DUMMY_EMAILS di bawah kalau ada yang baru)
    3. Kalau 1 orang submit berkali-kali, ambil submission PALING TERAKHIR aja
    4. Pisah feasible (fitness > 0) vs infeasible (fitness = 0 di PyGAD & DEAP)
    5. Hitung statistik: rata-rata, std, menang/seri/kalah, per kronotipe
...
# Kalau nemu identitas dummy/testing BARU selain 2 ini, tambahin emailnya di sini.
DUMMY_EMAILS = [
    "naurahan09@gmail.com",
    "nikican@gmail.com",   # "niki" -- dummy, dikonfirmasi bukan responden asli
    "zahira@gmail.com",    # "iraa" -- dummy, dikonfirmasi bukan responden asli
]
# ─────────────────────────────────────────────────────────────
...
def buat_boxplot_fitness(feasible):
    ...
    ax.set_title("Sebaran Fitness: PyGAD vs DEAP")
...
def main():
    ...
    # 1. Buang dummy
    df = df[~df["email"].isin(DUMMY_EMAILS)].reset_index(drop=True)
    print(f"Setelah buang dummy: {len(df)} baris, {df['email'].nunique()} email unik")

    # 2. Ambil submission terakhir per email
    idx = df.groupby("email")["created"].idxmax()
    final = df.loc[idx].sort_values("nama").reset_index(drop=True)
    final = final.drop(columns=["pemenang"])  # dihitung ulang manual biar konsisten
    final.insert(0, "ID", range(1, len(final) + 1))
    print(f"Setelah dedup (ambil terakhir per orang): {len(final)} responden unik")

    # 3. Pisah feasible vs infeasible
    is_infeasible = (final.fit_pygad == 0) & (final.fit_deap == 0)
    feasible = final[~is_infeasible].reset_index(drop=True)
    infeasible = final[is_infeasible].reset_index(drop=True)

    final.to_csv("data_responden_FINAL_bersih.csv", index=False)
    ...
    if feasible.empty:
        print("\nTidak ada data feasible, statistik PyGAD vs DEAP tidak bisa dihitung.")
        return
    ...
    print("  - data_responden_FINAL_bersih.csv  (semua responden, sudah bersih)")
    print("  - data_feasible_bab4.csv            (buat perbandingan PyGAD vs DEAP)")
    print("  - data_infeasible_temuan.csv        (buat temuan terpisah)")
    print("  - ringkasan_bab4.csv                (statistik per kronotipe)")
    print("  - boxplot_fitness.png")
    print("  - boxplot_durasi.png")
    print("  - fitness_per_kronotipe.png")
    print("  - kemenangan_algoritma.png")
